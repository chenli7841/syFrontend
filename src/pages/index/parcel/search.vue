			...
			<view class="list-li bfff" v-for="(item, index) in orderList" :key="index">
				...
				<view class="count">
					<view class="countO num-left">
						...
						<text class="text0 f24 c666">国内单号：{{ item.domesticnumber ? item.domesticnumber : '-' }}</text>
						<text v-if="type == '' || type == 100" class="textT f24 c666">国际单号：{{ item.secondtracknumber ? item.secondtracknumber : '-' }}</text>
					</view>
					<view class="countO num-left">
				...
				<view class="btn" v-if="type == 1">
					<text style="display: none;" @click="navtoGoots(item)" class="f24 cfff btnColor">改商品信息</text>
...
export default {
	...
	methods: {
		...
		navtoGoots(item) {
			uni.navigateTo({
				url: '../../order/two?isfromchina=' + item.isfromchina + '&id=' + item.id + '&put=1' + '&routeId=' + item.routeid
			});
		},
		changeInput(e, name) {
