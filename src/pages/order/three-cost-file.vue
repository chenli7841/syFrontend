<template>
	<view class="index bgColor">
		<view class="top">
			...
			<text class="c333  f28">运费公式备注和文件上传</text>
			<text class="c333 fb f24">(选填)</text>
		</view>
        <view class="card bfff">
			<view class="title f28 fb c333">运费公式</view>
			<view class="log log_lp" >
				<table style="border-spacing: 0;" class="c333 f24">
                    <tbody>
                        <tr v-for="(item, index) in itemPriceList" :key="index">
                            <td></td>
                            <td style="text-align: end;"><text>{{ item.name }} 计费重量</text></td>
                            <td style="padding: 0 16upx;"><text>&times;</text></td>
                            <td><text>{{ item.price }}</text></td>
                            <td><text>(基础运费)</text></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td></td>
                            <td style="text-align: end;"><text>计费总重量</text></td>
                            <td style="padding: 0 16upx;"><text>&times;</text></td>
                            <td><text>2</text></td>
                            <td><text>(超重附加费)</text></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td style="border-bottom: 1px black solid;"><text>+</text></td>
                            <td style="text-align: end; border-bottom: 1px black solid;"><text>{{  totalClaimPrice }}</text></td>
                            <td style="padding: 0 16upx; border-bottom: 1px black solid;"><text>&times;</text></td>
                            <td style="border-bottom: 1px black solid;"><text>0.01</text></td>
                            <td style="border-bottom: 1px black solid;"><text>(保险)</text></td>
...
export default {
	...
	onShow() {
        ...
        if (this.$parcelInfo.orderbaggageVoList && this.$parcelInfo.orderbaggageVoList[0] && this.$parcelInfo.orderbaggageVoList[0].itemVoList) {
            if (this.$parcelInfo && this.$parcelInfo.categoryPriceList) {
                this.itemPriceList = this.$parcelInfo.orderbaggageVoList[0].itemVoList.map(i => {
                    let c = this.$parcelInfo.categoryPriceList.find(p => p.Item === i.category);
                    if (c) {
                        return {
                            name: i.name,
                            category: i.category,
            ...
            this.totalClaimPrice = 0;
            this.$parcelInfo.orderbaggageVoList[0].itemVoList.forEach(i => {
                this.totalClaimPrice = this.totalClaimPrice +  Number(i.claimprice) * Number(i.count)
            });
        }
